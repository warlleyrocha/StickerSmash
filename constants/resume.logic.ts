...
import { mockRepublica, mockDividas } from "@/constants/resume.mock";
...
export const quantidadeContasPagas = mockRepublica.contas.filter((c) => c.pago).length;
export const quantidadeContasPendentes = mockRepublica.contas.filter((c) => !c.pago).length;
